<template>
    <div v-if="!getIssuesLoading">
        <div v-if="items">
            <div v-for="(item, index) in Object.keys(items)" :key="index" class="list-row">
                <div style="overflow: hidden; position: relative; text-align: center; width: 11.5%;" class="column ">
                    <div class="name">
                        {{item}}
                    </div>
                    <div>
                        <span style="">Active tasks </span> <span style="color: #4773BA; font-weight: 600; text-decoration: underline;">{{functions.countTasks(items[item])}}</span> 
                    </div>
